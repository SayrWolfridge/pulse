...
class DailyNoteSync:
    ...
    def __init__(self, config: PulseConfig):
        # Allow daily_notes to be either workspace-relative ("memory/")
        # or absolute ("~/.pulse/logs/daily-notes/" or "/var/log/pulse/...").
        daily = config.workspace.daily_notes
        daily_path = Path(daily).expanduser()

        if daily_path.is_absolute():
            # Absolute path: use as-is, don't join with workspace.root
            self.daily_dir = daily_path
        else:
            # Relative path: resolve under workspace root
            self.daily_dir = (
                Path(config.workspace.root).expanduser() / daily
            )
